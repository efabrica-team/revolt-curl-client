...
final class RevoltCurlClient implements HttpClientInterface, LoggerAwareInterface, ResetInterface
{
    use HttpClientTrait;
     ...
     */
    private RevoltCurlClientState $multi;
    ...
    public function request(string $method, string $url, array $options = []): ResponseInterface
    {
        ...
        } elseif (\defined('CURL_VERSION_HTTP2') && (\CURL_VERSION_HTTP2 & RevoltCurlClientState::$curlVersion['features']) && ('https:' === $scheme || 2.0 === (float)$options['http_version'])) {
        ...
        if (isset($options['auth_ntlm'])) {
            ...
            if (!\is_string($options['auth_ntlm'])) {
                throw new InvalidArgumentException(sprintf('Option "auth_ntlm" must be a string or an array, "%s" given.',
                    get_debug_type($options['auth_ntlm'])));
            }
        ...
        if ($options['resolve'] || $multi->dnsCache->evictions) {
            ...
            if ($resolve && 0x072A00 > RevoltCurlClientState::$curlVersion['version_number']) {
        ...
        if (!\is_string($body)) {
            ...
            if (isset($options['normalized_headers']['content-length'][0])) {
                $curlopts[\CURLOPT_INFILESIZE] = (int)substr($options['normalized_headers']['content-length'][0],
        ...
        foreach ($curlopts as $opt => $value) {
            if (null !== $value && !curl_setopt($ch, $opt,
                    $value) && \CURLOPT_CERTINFO !== $opt && (!\defined('CURLOPT_HEADEROPT') || \CURLOPT_HEADEROPT !== $opt)) {
                $constantName = $this->findConstantName($opt);
                throw new TransportException(sprintf('Curl option "%s" is not supported.', $constantName ?? $opt));
            }
        }

        return $pushedResponse ?? new RevoltCurlResponse($multi, $ch, $options, $this->logger, $method,
            self::createRedirectResolver($options, $host, $port), RevoltCurlClientState::$curlVersion['version_number'], $url);
    }

    public function stream(ResponseInterface|iterable $responses, ?float $timeout = null): ResponseStreamInterface
    {
        if ($responses instanceof RevoltCurlResponse) {
            $responses = [$responses];
        }

        foreach ($responses as $response) {
            if (!$response instanceof RevoltCurlResponse) {
                throw new TransportException('Responses must be instances of "RevoltCurlResponse".');
            }
        ...
        return new ResponseStream(RevoltCurlResponse::stream($responses, $timeout));
    }
     ...
     */
    private static function acceptPushForRequest(string $method, array $options, RevoltPushedResponse $pushedResponse): bool
    {
    ...
    private static function readRequestBody(int $length, \Closure $body, string &$buffer, bool &$eof): string
    {
        if (!$eof && \strlen($buffer) < $length) {
            if (!\is_string($data = $body($length))) {
                throw new TransportException(sprintf('The return value of the "body" option callback must be a string, "%s" returned.',
                    get_debug_type($data)));
            }
    ...
    private static function createRedirectResolver(array $options, string $host, int $port): \Closure
    {
        ...
        if (0 < $options['max_redirects']) {
            ...
            $redirectHeaders['with_auth'] = $redirectHeaders['no_auth'] = array_filter($options['headers'],
                static fn($h) => 0 !== stripos($h, 'Host:'));
            ...
            if (isset($options['normalized_headers']['authorization'][0]) || isset($options['normalized_headers']['cookie'][0])) {
                $redirectHeaders['no_auth'] = array_filter($options['headers'],
                    static fn($h) => 0 !== stripos($h, 'Authorization:') && 0 !== stripos($h, 'Cookie:'));
            }
        }
        ...
        return static function ($ch, string $location, bool $noContent) use (&$redirectHeaders, $options) {
            ...
            if ($noContent && $redirectHeaders) {
                $filterContentHeaders = static fn($h) => 0 !== stripos($h, 'Content-Length:') && 0 !== stripos($h,
                        'Content-Type:') && 0 !== stripos($h, 'Transfer-Encoding:');
    ...
    private function ensureState(): RevoltCurlClientState
    {
        $this->multi = new RevoltCurlClientState($this->maxHostConnections, $this->maxPendingPushes);
    ...
    {
        $constants = array_filter(get_defined_constants(),
            static fn($v, $k) => $v === $opt && 'C' === $k[0] && (str_starts_with($k, 'CURLOPT_') || str_starts_with($k, 'CURLINFO_')),
    ...
    private function validateExtraCurlOptions(array $options): void
    {
        $curloptsToConfig = [
            ...
            // options used in RevoltCurlResponse
        ...
        foreach ($options as $opt => $optValue) {
            if (isset($curloptsToConfig[$opt])) {
                ...
                throw new InvalidArgumentException(sprintf('Cannot set "%s" with "extra.curl", use option "%s" instead.', $constName,
                    $curloptsToConfig[$opt]));
            }
