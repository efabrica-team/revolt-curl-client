...
use Symfony\Component\HttpClient\Response\TransportResponseTrait;
 ...
 * @internal
 */
final class RevoltCurlResponse implements ResponseInterface, StreamableInterface
{
    ...
    public function __construct(
        ...
        if (!$info['response_headers']) {
            ...
            curl_setopt($ch, \CURLOPT_PRIVATE, \in_array($method, ['GET', 'HEAD', 'OPTIONS', 'TRACE'],
                true) && 1.0 < (float)($options['http_version'] ?? 1.1) ? 'H2' : 'H0'); // H = headers + retry counter
        }

        curl_setopt($ch, \CURLOPT_HEADERFUNCTION,
            static function ($ch, string $data) use (&$info, &$headers, $options, $multi, $id, &$location, $resolveRedirect, $logger): int {
        ...
        if ($onProgress = $options['on_progress']) {
            ...
            curl_setopt($ch, \CURLOPT_PROGRESSFUNCTION,
                static function ($ch, $dlSize, $dlNow) use ($onProgress, &$info, $url, $multi, $debugBuffer) {
                    try {
                        rewind($debugBuffer);
        ...
        curl_setopt($ch, \CURLOPT_WRITEFUNCTION, static function ($ch, string $data) use ($multi, $id): int {
            if ('H' === (curl_getinfo($ch, \CURLINFO_PRIVATE)[0] ?? null)) {
                ...
                $multi->handlesActivity[$id][] = new TransportException(sprintf('Unsupported protocol for "%s"',
    ...
    private static function perform(ClientState $multi, ?array &$responses = null): void
    {
        if ($multi->performing) {
            if ($responses) {
                ...
                $multi->handlesActivity[(int)$response->handle][] = new TransportException(sprintf('Userland callback cannot use the client nor the response while processing "%s".',
                    curl_getinfo($response->handle, \CURLINFO_EFFECTIVE_URL)));
            }
        ...
        try {
            ...
            while ($info = curl_multi_info_read($multi->handle)) {
                ...
                if (\in_array($result, [\CURLE_SEND_ERROR, \CURLE_RECV_ERROR, /* CURLE_HTTP2 */ 16, /* CURLE_HTTP2_STREAM */ 92],
                ...
                $multi->handlesActivity[$id][] = null;
                $multi->handlesActivity[$id][] = \in_array($result, [\CURLE_OK, \CURLE_TOO_MANY_REDIRECTS],
                    true) || '_0' === $waitFor || curl_getinfo($ch, \CURLINFO_SIZE_DOWNLOAD) === curl_getinfo($ch,
                    \CURLINFO_CONTENT_LENGTH_DOWNLOAD) ? null : new TransportException(ucfirst(curl_error($ch) ?: curl_strerror($result)) . sprintf(' for "%s".',
                        curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL)));
            }
        } finally {
    ...
    private static function select(ClientState $multi, float $timeout): int
    {
        ...
        $timeLeft = $timeout;
        do {
            $selected = curl_multi_select($multi->handle, 0);
            if ($selected !== 0) {
                return $selected;
            }
            delay(self::DELAY);
     ...
     */
    private static function parseHeaderLine(
        $ch,
        string $data,
        array &$info,
        array &$headers,
        ...
        if ("\r\n" !== $data) {
            ...
            if (300 <= $info['http_code'] && $info['http_code'] < 400) {
                ...
                } elseif (303 === $info['http_code'] || ('POST' === $info['http_method'] && \in_array($info['http_code'], [301, 302],
                            true))) {
        ...
        if (300 <= $statusCode && $statusCode < 400 && null !== $location) {
            ...
            if (null === $info['redirect_url'] = $resolveRedirect($ch, $location, $noContent)) {
                ...
                if (isset($url['host']) && null !== $ip = $multi->dnsCache->hostnames[$url['host'] = strtolower($url['host'])] ?? null) {
                    ...
                    $port = $url['port'] ?? ('http' === ($url['scheme'] ?? parse_url(curl_getinfo($ch, \CURLINFO_EFFECTIVE_URL),
                        \PHP_URL_SCHEME)) ? 80 : 443);
        ...
        } elseif ($statusCode < 300 || 400 <= $statusCode || null === $location || curl_getinfo($ch,
                \CURLINFO_REDIRECT_COUNT) === $options['max_redirects']) {
